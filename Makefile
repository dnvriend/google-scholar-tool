...
typecheck: ## Run type checking with mypy
	uv run mypy google_scholar_tool

test: ## Run tests
	uv run pytest tests/

security-bandit: ## Run bandit security linter
	uv run bandit -r google_scholar_tool -c pyproject.toml
...
run: ## Run google-scholar-tool (usage: make run ARGS="...")
...
install-global: build ## Install globally with uv tool (rebuilds first to avoid cache issues)
	-uv tool uninstall google-scholar-tool 2>/dev/null
	uv tool install . --reinstall

uninstall-global: ## Uninstall global installation
	uv tool uninstall google-scholar-tool
